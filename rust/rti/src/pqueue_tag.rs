...
pub struct PQueueTag {
    queue: PriorityQueue<Tag, usize>,
}

impl PQueueTag {
    pub fn new() -> PQueueTag {
        PQueueTag {
            queue: PriorityQueue::with_capacity(10),
        }
    }

    pub fn pqueue_tag_insert_if_no_match(_f_rti: Arc<RwLock<RTIRemote>>, fed_id: u16, t: Tag) {
        match Self::pqueue_tag_find_with_tag(_f_rti.clone(), fed_id, t.clone()) {
            Some((t, _p)) => {}
            None => {
                Self::pqueue_tag_insert_tag(_f_rti, fed_id, t);
            }
        }
    }

    fn pqueue_tag_find_with_tag(
        ...
        let q = fed.in_transit_message_tags();
        match q.queue().get(&t) {
    ...
    fn pqueue_tag_insert_tag(_f_rti: Arc<RwLock<RTIRemote>>, fed_id: u16, t: Tag) {
        let mut locked_rti = _f_rti.write().unwrap();
        let idx: usize = fed_id.into();
        let fed: &mut FederateInfo = &mut locked_rti.base_mut().scheduling_nodes_mut()[idx];
        let q = fed.in_transit_message_tags_mut();
        q.queue_mut().push(t, 0);
    }

    pub fn pqueue_tag_remove_up_to(_f_rti: Arc<RwLock<RTIRemote>>, fed_id: u16, t: Tag) {
        loop {
            let head = Self::pqueue_tag_peek_tag(_f_rti.clone(), fed_id);
            if Tag::lf_tag_compare(&head, &Tag::forever_tag()) >= 0
                || Tag::lf_tag_compare(&head, &t) > 0
            {
                break;
            }
            Self::pqueue_tag_pop(_f_rti.clone(), fed_id);
        }
    }

    pub fn pqueue_tag_peek_tag(_f_rti: Arc<RwLock<RTIRemote>>, fed_id: u16) -> Tag {
        let locked_rti = _f_rti.read().unwrap();
        let idx: usize = fed_id.into();
        let fed = &locked_rti.base().scheduling_nodes()[idx];
        let pqueue_tag = fed.in_transit_message_tags();
    ...
    fn pqueue_tag_pop(_f_rti: Arc<RwLock<RTIRemote>>, fed_id: u16) {
        let mut locked_rti = _f_rti.write().unwrap();
        let idx: usize = fed_id.into();
        let fed: &mut FederateInfo = &mut locked_rti.base_mut().scheduling_nodes_mut()[idx];
        let q = fed.in_transit_message_tags_mut();
